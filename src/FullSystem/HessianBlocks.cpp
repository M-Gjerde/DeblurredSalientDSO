                              ...
                              SaliencyUtil::SaliencyContainer* blur_container)
{
	...
	if (saliency_container != NULL) {
		// Copy saliency
		if (saliency_container->saliency_ != NULL) {
			if (saliency_ != NULL) {
				delete [] saliency_;
			}
			saliency_ = new float [w * h];

			float* saliency = saliency_container->saliency_->image;
        	for (int i = 0; i < (w * h); i++) {
        	    saliency_[i] = saliency[i];
        	}
    	}

    	// Copy segmentation
    	if (saliency_container->segmentation_ != NULL) {
			if (segmentation_ != NULL) {
				delete [] segmentation_;
			}
			segmentation_ = new unsigned char [w * h];

			float* segmentation =
				saliency_container->segmentation_->image;
        	for (int i = 0; i < (w * h); i++) {
        	    segmentation_[i] = static_cast<unsigned char>(segmentation[i]);
        	}
    	}
    }

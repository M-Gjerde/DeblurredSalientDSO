...
Vec4 FullSystem::trackNewCoarse(FrameHessian* fh)
{
	...
	// To visualize all frame, uncomment this.
	// But you can visualize saliency and features' selection then.
...
void FullSystem::makeKeyFrame( FrameHessian* fh)
{
    ...
    for(IOWrap::Output3DWrapper* ow : outputWrapper)
    {
        ...
        // Only keyframes will be visualized.
        ow->pushLiveFrame(fh);
    }
...
void FullSystem::makeNewTraces(FrameHessian* newFrame, float* gtDepth)
{
	...
	// In order to visualize pixels' selection, change false to true.
	int numPointsTotal = pixelSelector->makeMaps(newFrame, selectionMap,
		setting_desiredImmatureDensity, 1, setting_visualize_selection);
    ...
    newFrame->debugImage = new MinimalImageB3(w, h);
    // MinimalImageB3 img(w,h);
    MinimalImageB3* img = newFrame->debugImage;

    for(int i=0;i<w*h;i++)
    {
      float c = newFrame->dI[i][0]*0.7;
    ...
    for(int y=0; y<h;y++) {
      for(int x=0;x<w;x++)	{
        ...
        else if(selectionMap[i] == 2)
          img->setPixelCirc(x,y,Vec3b(255,0,0));
        else if(selectionMap[i] == 4)

...
private:
	...
	void makeSaliencyHists(const FrameHessian* const fh);

	void setUpSaliencyRandomSampler();

	int getSaliencyWeightedRandomNumber();
	...
	int* saliency_hist_;
	int* saliency_ths_;
	float* saliency_ths_smoothed_;
	float max_saliency_;
	float min_saliency_;

    std::mt19937* gen_;
    std::discrete_distribution<>* saliency_random_;

    SaliencyUtil::SaliencySmoother* saliency_smoother_;
};

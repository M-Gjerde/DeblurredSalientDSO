...
void PangolinDSOViewer::run()
{
	...
	// 3 images
	pangolin::View& d_kf = pangolin::Display("imgKF")
	    .SetAspect(w/(float)h);
	...
	pangolin::Var<bool> settings_showLiveKF("ui.showKF",true,true);
	pangolin::Var<bool> settings_showLiveSaliency("ui.showSaliency",true,true);
    pangolin::Var<bool> settings_showLiveSelection("ui.showSelection",true,true);
	...
	pangolin::Var<double> settings_saliencyHistAdd("ui.minSaliencyAdd",setting_minSaliencyHistAdd,0,256, false);
	...
	while( !pangolin::ShouldQuit() && running )
	{
		...
		if(setting_render_displaySaliency)
		{
			d_saliency.Activate();
		...
		if(setting_render_displayKF)
		{
			d_kf.Activate();
			glColor4f(1.0f,1.0f,1.0f,1.0f);
			texKF.RenderToViewportFlipY();
		}

		if(setting_render_displaySelection)
		{
			d_selection.Activate();
			glColor4f(1.0f,1.0f,1.0f,1.0f);
			texSelection.RenderToViewportFlipY();
		}
		...
		setting_render_displayKF = settings_showLiveKF.Get();
		setting_render_displaySaliency =  settings_showLiveSaliency.Get();
		setting_render_displaySelection = settings_showLiveSelection.Get();
	    ...
	    setting_minSaliencyHistAdd = settings_saliencyHistAdd.Get();
...
void PangolinDSOViewer::pushLiveFrame(FrameHessian* image)
{
    ...
    if (setting_render_displaySaliency && image->saliency_ != NULL) {
	    ...
	    for(int i=0;i<w*h;i++) {
		    ...
		    internalSaliencyImg->data[i][2] = result_mat.at<cv::Vec3b>( i / w, i % w)[2];
